...
def cost_function(theta, X, y):
    ...
    # to avoid warning even if calcul are handled when divided by 0 or by +inf
...
def sigmoid(z):
    # to avoid warning and inf values
    z[z > 700] = 700
    z[z < -700] = -700
...
def gradient_descent(X, y, alpha=0.001, num_iter=10000, tol=0.01):
    ...
    for i in range(num_iter):
        ...
        # save best cost find during algo
...
#     cost_test = np.finfo(np.float64).max
#     print(f"cost = {cost_save}")
#     theta_save = theta.copy()
#     grad_save = calculate_gradient(theta, X_b, y)
...
#         # check if a cost has been better during algo
#         cost = cost_function(theta, X_b, y)
#         # print(f"cost = {cost}")
